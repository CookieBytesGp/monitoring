...
{
    internal class CameraConfiguration : IEntityTypeConfiguration<Domain.Aggregates.Camera.Camera>
    {
        public void Configure(EntityTypeBuilder<Domain.Aggregates.Camera.Camera> builder)
        {
            // Table Configuration
            ...
            // Configuration Entity (Complete Configuration as Owned Entity)
            builder.OwnsOne(p => p.Configuration, config =>
            {
                ...
                config.OwnsOne(c => c.MotionDetection, motion =>
                {
                    motion.Property(m => m.IsEnabled)
                        .IsRequired(true)
                        .HasColumnName("Configuration_MotionDetection_IsEnabled");
                    ...
                    motion.Property(m => m.DetectionZone)
                        .HasMaxLength(500)
                        .IsRequired(false)
                        .HasColumnName("Configuration_MotionDetection_DetectionZone");
                });
                ...
                config.OwnsOne(c => c.Recording, recording =>
                {
                    recording.Property(r => r.IsEnabled)
                        .IsRequired(true)
                        .HasColumnName("Configuration_Recording_IsEnabled");
                        
                    recording.Property(r => r.Quality)
                        .IsRequired(true)
                        .HasColumnName("Configuration_Recording_Quality")
                        .HasConversion<string>();
                        
                    recording.Property(r => r.Duration)
                        .IsRequired(true)
                        .HasColumnName("Configuration_Recording_Duration");
            ...
            builder.OwnsMany(p => p.Capabilities, capabilities =>
            {
                ...
                capabilities.Property(c => c.Description)
                    .HasMaxLength(200)
                    .IsRequired(false);
